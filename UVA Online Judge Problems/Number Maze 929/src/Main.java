...
public class Main {
	public static void main(String [] args){
		...
		for(int i = 0; i < cases; i++){
			...
			int rows = input.nextInt();input.nextLine();
			int cols = input.nextInt();input.nextLine();
			int [][] map = new int[rows][cols];
			for(int p = 0; p < rows; p ++){
				for(int j = 0; j < cols; j++){
					map[p][j] = input.nextInt();
				}
				input.nextLine();
			}
			System.out.println(navigate(0, 0, map.length - 1, map[0].length, map));
		}
	}
	static int navigate(int startx, int starty,int goalx, int goaly, int [][] grid){
		int [][] delta = {{1, 0},
						{ 0, 1},
						{-1, 0},
						{ 0,-1}};
